...
import { Plus, Minus } from "../../constants/icons";
import styles from "./index.module.scss";
import { useDispatch, useSelector } from "react-redux";
import { increaseQuantity, decreaseQuantity } from "../../features/cart-slice";
...
const Counter = ({
  ...
  const handleClick = (type) => {
    if (type === "increase") {
      if (from === "cart") {
        if (count + 1 > item.maxQuantity) {
          message.warn(t("NotAllowed"));
        } else {
          setCount(count + 1);
          setDefaultVal(count + 1);
          let data = item;
          dispatch(
            increaseQuantity({
              ...
              companyLogo: data.company?.logo
                ...
                : fallBackImage,
              type: data.source,
              categoryId: data.productName ? 0 : data.category?.id,
            })
          );
        }
      } else {
